    ...
    for drv in session.drivers:
        ...
        for _, lap in laps.iterrows():
            try:
                ...
                continue
    if records:
        return pd.concat(records, ignore_index=True), None
    # Return the first error message to help diagnose
    err_msg = errors[0] if errors else "No position data found in any lap"
    return pd.DataFrame(columns=["X","Y","Time","Driver","LapNumber","Num","Team"]), err_msg
...
def build_tel_figure(session, drv1, drv2):
    ...
    def get_tel(drv):
        try:
            dlaps = session.laps.pick_drivers(drv)
            if dlaps.empty:
                tel_errors.append(f"{drv}: no laps found")
                return None, None
            lap = dlaps.pick_fastest()
            if lap is None:
                tel_errors.append(f"{drv}: pick_fastest returned None")
                return None, None
            car_data = lap.get_car_data()
            if car_data is None or car_data.empty:
                tel_errors.append(f"{drv}: car_data empty")
                return None, None
            tel = car_data.add_distance()
            return tel, lap
        except Exception as e:
            tel_errors.append(f"{drv}: {str(e)[:80]}")
            return None, None
...
def render_timing(session, stype):
    ...
    # Alerts - only show if flag was ACTIVE at the END of the session
    try:
        ...
        if msgs is not None and not msgs.empty:
            try:
                ...
                max_lap = 999
            # Only look at last 2 laps worth of messages
            lap_col = msgs.get("Lap") if "Lap" in msgs.columns else None
            if lap_col is not None:
                end_msgs = msgs[msgs["Lap"].fillna(0) >= max(1, max_lap - 1)]
                if end_msgs.empty:
                    end_msgs = msgs.tail(3)
            else:
                end_msgs = msgs.tail(3)

            active_flag = None
            for _, m in end_msgs.sort_values("Time").iterrows():
                ...
                if "SAFETY CAR DEPLOYED" in txt:
                    ...
                    active_flag = "red"
                elif any(w in txt for w in ["SAFETY CAR IN THIS LAP","CLEAR","GREEN LIGHT"]):
                    active_flag = None

            if active_flag == "sc":
                st.markdown('<div class="alert-sc">Safety Car</div>',
                            unsafe_allow_html=True)
            elif active_flag == "vsc":
                st.markdown('<div class="alert-sc">Virtual Safety Car</div>',
                            unsafe_allow_html=True)
            elif active_flag == "red":
...
def render_map(session, session_key):
    ...
    if (st.session_state.get("map_session_key") != session_key or
        ...
        with st.spinner("Loading position data (this may take 1-2 min the first time)..."):
            st.session_state["circuit_xy"]      = build_circuit_xy(session)
            pos_data, pos_err                   = build_driver_positions(session)
            st.session_state["driver_pos"]      = pos_data
            st.session_state["driver_pos_err"]  = pos_err
            st.session_state["map_session_key"] = session_key

    circuit_xy = st.session_state["circuit_xy"]
    all_pos    = st.session_state["driver_pos"]
    pos_err    = st.session_state.get("driver_pos_err")

    if all_pos.empty:
        msg = (f"GPS position data could not be loaded. "
               f"This can happen with very recent 2026 sessions where FastF1 "
               f"has not yet processed the full telemetry stream.")
        if pos_err:
            msg += f" (Detail: {pos_err[:120]})"
        st.warning(msg)
        st.info("Try: Circuit Map works best after FastF1 has had a few hours "
                "to process the session data. Race sessions have the most complete GPS data.")
        return
...
def render_telemetry(session, session_key):
    ...
    if not has_data:
        st.warning(
            "Telemetry could not be loaded for this session. "
            "This happens with very recent races where FastF1 is still "
            "processing the car data stream."
        )
        st.caption(
            "Tip: Wait a few hours after the session ends, delete the cache/ folder, "
            "and reload. Race and Qualifying sessions from 2024/2025 work reliably."
        )
        return
